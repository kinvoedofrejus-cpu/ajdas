  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (pathname === "/api/items" && request.method === "GET") {
        ...
        let query = "SELECT id, type, title, description, filename, r2_key, mime_type, created_at FROM items";
        const params = [];
      ...
      if (pathname === "/api/items" && request.method === "POST") {
        ...
        if (!MULTI_FILE_TYPES.includes(type)) {
          ...
          await env.DB.prepare(
            `INSERT INTO items (id, type, title, description, filename, r2_key, mime_type, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
          ).bind(id, type, commonTitle, description, filename, r2_key, mime_type, Date.now()).run();
          return json({ items: [{ id, type, title: commonTitle, description, filename, r2_key, mime_type, created_at: Date.now() }] }, 201);
        }
        ...
        for (const file of allFiles) {
          ...
          const title = allFiles.length > 1
            ? (commonTitle ? `${commonTitle} — ${cleanTitleFromFilename(filename)}` : cleanTitleFromFilename(filename))
            : (commonTitle || cleanTitleFromFilename(filename));

          const created_at = Date.now();
          await env.DB.prepare(
            `INSERT INTO items (id, type, title, description, filename, r2_key, mime_type, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
          ).bind(id, type, title, description, filename, r2_key, mime_type, created_at).run();
          created.push({ id, type, title, description, filename, r2_key, mime_type, created_at });
        }
