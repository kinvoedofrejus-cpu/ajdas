...
// (projets, documents, galerie). Les pages statiques (public/) sont
...
const ALLOWED_TYPES = ["project", "document", "image"];
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (pathname === "/api/items" && request.method === "POST") {
        ...
        if ((type === "document" || type === "image") && (!file || typeof file === "string")) {
          return json({ error: "Un fichier est requis pour ce type de contenu." }, 400);
        }
        ...
        if (file && typeof file !== "string") {
          ...
          await env.FILES.put(r2_key, await file.arrayBuffer(), {
        ...
        await env.DB.prepare(
          `INSERT INTO items (id, type, title, description, filename, r2_key, mime_type, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
        ).bind(id, type, title, description, filename, r2_key, mime_type, Date.now()).run();

        return json({ id, type, title, description, filename, r2_key, mime_type, created_at: Date.now() }, 201);
      }
