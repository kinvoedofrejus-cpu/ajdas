...
## Mise à jour de ta base existante

Si ton site est déjà déployé, exécute ces commandes dans le dashboard Cloudflare → D1 → ta base (`ajdas_db`) → onglet **Console**, une par une. Si tu as déjà exécuté l'une d'elles lors d'une mise à jour précédente, ignore-la (D1 renverra une erreur "duplicate column" si tu la relances, sans conséquence).

**Option "téléchargement autorisé"** (si pas encore fait) :
```sql
ALTER TABLE items ADD COLUMN downloadable INTEGER NOT NULL DEFAULT 1;
```

**Regroupement des fichiers envoyés ensemble** :
```sql
ALTER TABLE items ADD COLUMN group_id TEXT;
```

Aucune de ces commandes ne supprime de contenu existant.
