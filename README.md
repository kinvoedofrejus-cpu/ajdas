...
## Mise à jour de ta base existante (colonne "téléchargement autorisé")

Si ton site est déjà déployé, ta base D1 ne connaît pas encore l'option d'autoriser ou non le téléchargement. Va dans le dashboard Cloudflare → D1 → ta base (`ajdas_db`) → onglet **Console**, colle ceci puis exécute :

```sql
ALTER TABLE items ADD COLUMN downloadable INTEGER NOT NULL DEFAULT 1;
```

Cette commande ne supprime rien : tout le contenu déjà publié reste "téléchargeable" par défaut, et tu peux désactiver le téléchargement au cas par cas depuis l'espace admin en publiant ou modifiant un contenu.
